...
export function createSearchFlowController({
  ...
  function updateSummary(query, meta) {
    ...
    setSummary(`Showing ${visibleResults} result(s) for "${query}".`);
  }
  ...
  async function loadMoreResults() {
    ...
    try {
      ...
      if (!payload.items.length && !visibleResults) {
        renderEmptyState({
          title: meta.error === "No results found." ? "No titles found" : "Unable to load titles",
          tone: meta.error === "No results found." ? "neutral" : "error"
        });
        setStatus(meta.error || "No results found.", meta.error === "No results found." ? "default" : "error");
